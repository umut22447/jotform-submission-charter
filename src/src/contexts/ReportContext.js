...
export const ReportProvider = ({ children, formId }) => {
        ...
        getSubmissionQuestionsById(formId)
            .then(questions => {
                ...
                const newReport = chartArr.map(field => {
                    ...
                    let reportObject = { "field": field, "title": title, "chartType": "Pie" };
                    return reportObject;
                })
    ...
    return (
        <ReportContext.Provider value={{ report, answers, form, deleteChartByField, deletedReport, changeChartTypeByField }}>
            {children}
